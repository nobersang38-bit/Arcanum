...
namespace Arcanum {
	namespace Skills {
		...
		namespace SkillVFX { // 이펙트 효과용 0을 기준으로 +버프, -디버프 => 추후 스킬컴포넌트에서 스택도 관리함
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(None);            // 없음
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Bleed);           // 출혈
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Mark);            // 표식
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Aegis);           // 수호
	...
	namespace Items {
		namespace Weapon {
			namespace Common {      // 일반 무기
				ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(GreatSword);
				ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Staff);
				ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(Bow);
		...
		namespace SetItem {        //  세트 아이템
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ManaCycle);        // 마나 재생 속도 증가 
		...
		namespace SetBonus
		{
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ManaCycle);
			ARCANUM_API UE_DECLARE_GAMEPLAY_TAG_EXTERN(ItemEffectBonus);
		}
		*/
	}
}
